    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ibMyLove.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                List<Collection> MyCollections=dbHelper.readMyCollections(stuId);
                boolean isDuplicate = false;
                for (Collection collections : MyCollections) {
                    if (collections.getTitle().equals(collection.getTitle()) &&
                            collections.getDescription().equals(collection.getDescription()) &&
                            collections.getPrice()==(collection.getPrice())) {
                        isDuplicate = true;
                        break;
                    }
                }
                if (isDuplicate) {
                    Toast.makeText(ReviewCommodityActivity.this, "该商品已经收藏过了！", Toast.LENGTH_SHORT).show();
                } else {
                    // 将新的收藏项添加到数据库和列表中
